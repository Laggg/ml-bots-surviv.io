...
6. Данные есть, сервер для вычислений есть, идеи есть, а рук - не хватает, поэтому - присоединяйтесь
7. Уже есть некоторые наработки по нейронному движку (но нужно улучшать). Ниже пример одной из моделей, которая возвращает следующий кадр рандомному агенту (который случайным образом выбирает направление своего движения): слева - начальный кадр, справа - некоторая траектория агента
<div align="center">
