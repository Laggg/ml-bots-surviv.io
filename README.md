...
1. Игра surviv.io не имеет окружения, в котором можно было бы обучить RL-агента. Окружение, оно же environment - функция, принимающая в себя текущее состояние среды и действие агента, а возвращающая - следующее состояние и награду. Т.е. S_next,Reward=ENV(S_curr,Action).
2. Текущая версия агента получена с помощью алгоритмов offline reinforcement learning
3. Идея - создать модель, выполняющую функции окружения, которая бы предсказывала следующий кадр, если известен текущий кадр и действие агента, т.е. State_next = MODEL(State_curr,Action)
...
6. После создания модели - нейронного движка игры, можно получить более сильного агента (научить лутать, правильно двигаться)
7. Данные есть, сервер для вычислений есть, идеи есть, а рук - не хватает, поэтому - присоединяйтесь
8. Уже есть некоторые наработки (без применения GameGAN) по нейронному движку (но нужно улучшать). Ниже пример одной из моделей, которая возвращает следующий кадр агенту: слева - начальный кадр, справа - некоторая траектория рандомного агента (который случайным образом выбирает направление своего движения)
<div align="center">
