...
2. Идея - создать модель, которая бы моделировала следующий кадр, если известен текущий кадр и действие агента, т.е. State_next = MODEL(State_curr,Action)
...
5. Данные есть, сервер для вычислений есть, идеи есть, а рук - не хватает. Поэтому - присоединяйтесь, будем рады. 
